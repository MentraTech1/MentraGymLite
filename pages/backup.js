window.Module_Backup = function({ gymId, userId, showToast }) {
    const { useState, useRef, useEffect } = React;

    const [isLoading, setIsLoading] = useState(false);
    const [loadingText, setLoadingText] = useState("جاري المعالجة..."); // نص ديناميكي لطمأنة المستخدم
    const [lastBackup, setLastBackup] = useState(localStorage.getItem(`MentraGym_LastBackup_${gymId}`) || 'لم يتم عمل نسخة بعد');
    ...
    const handleExport = async () => {
        setIsLoading(true);
        setLoadingText("جاري تجميع بيانات الجيم...");
        await yieldToMain();
        ...
        try {
            ...
            downloadAnchorNode.download = `MentraGym_Backup_${dateStr}.json`;
            ...
            localStorage.setItem(`MentraGym_LastBackup_${gymId}`, now);
            setLastBackup(now);
    ...
    const handleImport = (event) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (importedData.app !== "MentraGym" || !importedData.data) {
    ...
    const handleFactoryReset = async () => {
        if (!confirm("🚨 تحذير خطير جداً 🚨\nهل أنت متأكد من رغبتك في مسح كافة بيانات الجيم (الأعضاء، التمارين، الإعدادات)؟ لا يمكن التراجع عن هذه الخطوة!")) return;
        
        // أسلوب تأكيد أفضل للموبايل (بدون كتابة text)
        ...
        try {
            ...
            setTimeout(() => {
                localStorage.removeItem('MentraGym_Session');
    ...
    return (
        // تم إضافة pb-28 هنا لضمان عدم تغطية المحتوى بواسطة شريط الموبايل السفلي
        <div className="space-y-6 fade-up pb-28 md:pb-8 relative max-w-5xl mx-auto">
            
            {/* مؤشر التحميل (Loader) */}
            {isLoading && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4">
                    <div className="bg-white p-6 md:p-8 rounded-3xl shadow-2xl flex flex-col items-center text-center max-w-sm w-full">
                        ...
                        <p className="text-[10px] text-rose-500 mt-4 font-bold bg-rose-50 px-3 py-1 rounded-full">لا تقم بإغلاق المتصفح أو التطبيق</p>
                    </div>
                </div>
            )}
            ...
            <div className="bg-gradient-to-l from-[#06B6D4] to-[#3B82F6] rounded-3xl p-6 sm:p-8 text-white shadow-lg relative overflow-hidden">
                ...
                <p className="text-white/80 text-xs md:text-sm font-bold relative z-10 max-w-xl">
                    حافظ على أمان بيانات صالتك الرياضية عبر تحميل نسخ احتياطية دورية واسترجاعها وقت الحاجة.
                </p>
                <div className="mt-4 inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-4 py-2 rounded-xl text-xs md:text-sm font-bold relative z-10">
                    <i className="fas fa-clock"></i>
                    آخر نسخة: <span dir="ltr" className="ml-1">{lastBackup}</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col items-center text-center">
                    <div className="w-16 h-16 rounded-2xl bg-cyan-50 text-[#06B6D4] flex items-center justify-center text-2xl mb-4">
                    ...
                    <p className="text-slate-500 text-xs md:text-sm font-bold mb-6">
                        يقوم بتحميل ملف يحتوي على جميع بيانات الأعضاء، التمارين، والاشتراكات لحفظها على جهازك.
                    </p>
                    <button 
                        onClick={handleExport}
                        className="w-full mt-auto bg-slate-900 text-white font-black py-4 rounded-2xl hover:bg-slate-800 transition-colors flex justify-center items-center gap-2 active:scale-95"
                    >
                ...
                <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col items-center text-center">
                    <div className="w-16 h-16 rounded-2xl bg-blue-50 text-[#3B82F6] flex items-center justify-center text-2xl mb-4">
                        <i className="fas fa-upload"></i>
                    </div>
                    <h3 className="text-lg font-black text-slate-800 mb-2">استرجاع نسخة سابقة</h3>
                    <p className="text-slate-500 text-xs md:text-sm font-bold mb-6">
                        قم برفع ملف النسخة الاحتياطية الذي قمت بتحميله مسبقاً لاستعادة البيانات. (سيتم استبدال البيانات الحالية).
                    </p>
			...
			<div className="bg-rose-50 rounded-3xl p-5 md:p-6 border border-rose-100 flex flex-col sm:flex-row items-center gap-4 justify-between text-center sm:text-right">
                <div>
                ...
                <button 
                    ...
                    className="w-full sm:w-auto px-6 py-3 bg-rose-600 text-white text-sm font-black rounded-xl hover:bg-rose-700 transition-colors flex-shrink-0 shadow-md active:scale-95"
                >
                    مسح جميع البيانات
                </button>
            </div>
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl border border-[#06B6D4]/30 group mt-4">
                <div className="absolute top-0 right-0 w-64 h-64 bg-[#06B6D4]/10 rounded-full blur-[60px] pointer-events-none"></div>
                <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-6">
                    <div className="flex-1 text-center lg:text-right text-white">
                        ...
                        <p className="text-slate-400 font-bold text-xs sm:text-sm leading-relaxed max-w-2xl mx-auto lg:mx-0">
                            لا حاجة لأخذ نسخ احتياطية يدوياً بعد الآن. في نسخة MentraGym PRO تُحفظ بياناتك لحظياً على خوادم سحابية آمنة، ويمكنك متابعة صالتك الرياضية من أي جهاز في العالم.
                        </p>
                    </div>
